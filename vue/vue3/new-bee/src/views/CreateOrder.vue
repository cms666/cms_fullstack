<template>
  <div class="create-order">
    <!-- 头部 -->
    <s-header :name="'生成订单'" :back="'/cart'"></s-header>
    <!-- 地址 -->
...
export default {
  ...
  setup() {
    ...
    const init = async () => {
      ...
      const _cartItemIds = cartItemId
        ? JSON.parse(cartItemId)
        : JSON.parse(getLocal("cartItemId"));
      setLocal("cartItemId", JSON.stringify(_cartItemIds));
      //请求用户地址
    ...
    const goTo = () => {
      ...
      router.push({
        path: "/address",
        query: {
          cartItemId: JSON.stringify(state.cartItemId),
          from: "create-order",
        },
      });
    };
